...
internal class JacksonLibAssistedOperationsUnitTest {
    @Test
    fun when_file_is_read_then_domain_objects_are_populated() {
        val goods = readCsv(javaClass.getResourceAsStream("../apache/taxables.csv")!!)
    ...
    fun when_domain_collection_is_written_then_valid_csv_is_produced() {
        val goods = readCsv(javaClass.getResourceAsStream("../apache/taxables.csv")!!)
        val renderedCsv = ByteArrayOutputStream().apply { writeCsv(goods) }.toByteArray().let { String(it) }
        val expected = """
